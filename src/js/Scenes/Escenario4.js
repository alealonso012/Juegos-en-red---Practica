...
//FIN COSAS PERSONAJE
    ...
    create() {
        ...
        this.suelo2 = this.add.rectangle(544, 384, 128, 32, 0xffffff, 0.0).setOrigin(0)
        this.physics.add.existing(this.suelo2)
        this.suelo2.body.setImmovable(true)
        this.suelo2.body.allowGravity = false

        this.suelo3 = this.add.rectangle(160, 368, 176, 32, 0xffffff, 0.0).setOrigin(0)
        this.physics.add.existing(this.suelo3)
        this.suelo3.body.setImmovable(true)
        this.suelo3.body.allowGravity = false

        this.pared = this.add.rectangle(160, 512-144, 32, 144, 0xffffff, 0.0).setOrigin(0)
        this.physics.add.existing(this.pared)
        this.pared.body.setImmovable(true)
        ...
        this.physics.add.collider(this.player2, this.pared)
    }
    ...
    telonComprobarCerrado(Derecha) {
        if (Derecha) {
            if (this.telon.body.x > 0) {
                this.telon.body.setVelocityX(0);
                this.scene.start("Escenario3", { derecha: true});
            }
        }
        else {
            if (this.telon.body.x < 0) {
                this.telon.body.setVelocityX(0);
                this.scene.start("Escenario5", { derecha: false});
            }
        }
    }
}
